...
export const ImportResults = ({ results }: ImportResultsProps) => {
  ...
  return (
    <div className="space-y-6 mb-6">
      {/* Resumen General */}
      <div className={`p-4 rounded-lg border ${results.creados.length > 0 ? 'bg-green-50 border-green-200' : 'bg-red-50 border-red-200'}`}>
              ...
              <tbody className="divide-y divide-gray-100">
                {results.creados.map((item, idx) => (
                  <tr key={idx} className="hover:bg-gray-50">
                    <td className="px-4 py-2 font-mono text-xs text-gray-500">{item.tracking_id}</td>
